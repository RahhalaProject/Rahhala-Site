...
export class PasswordRecoveryOTPComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.fromForgotPassword = !!state?.fromForgotPassword;
  ...
  resendOtp(): void {
    ...
    this.authService
      ...
      .subscribe({
        ...
        error: (error) => {
          this.errorMessage.set(extractApiError(error, 'Failed to resend OTP. Please try again.'));
          this.isResending.set(false);
        },
      });
  }
  ...
  onSubmit(): void {
    ...
    const verification$ = this.fromForgotPassword
